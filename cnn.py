...
import pickle
LOG_FOUT = open(os.path.join('./log', 'log_cnn_1.txt'), 'w')
...
class RumorClassifier(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc = nn.Sequential(nn.Linear(256 * 3+43, 1),nn.Sigmoid())
#         self.fc = nn.Sequential(nn.Linear(256 * 3+43, 1),
#                   nn.ReLU(),
#                   nn.Dropout(0.3),
#                  nn.Linear(128,64),
#                   nn.ReLU(),
...
#                   nn.Sigmoid()
#                  )
            #输入的最后一个维度，输出的最后一个维度 全连接层只改变数据的最后一个维度 由输入最后的一个维度转化为 类别数
    ...
    def forward(self, seq_,attn_masks, seg_, stats):
        ...
        encoder_out = self.bert_layer(seq_,attention_mask=attn_masks, token_type_ids = seg_,return_dict=True).last_hidden_state
        out = encoder_out.unsqueeze(1) 
        
        out = torch.cat([self.conv_and_pool(out,conv) for conv in self.convs],1)
        x = torch.cat((out,stats),dim=1)
        out=self.fc(x) # 128*10 通道数=n-gram数*每种n-gram内的卷积数
